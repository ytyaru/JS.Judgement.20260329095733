...
import { Judgement, JudgementError, JudgementImplementationError, JudgementAsyncError, JudgementFailError, JudgementResultError, Pass, Fail } from "./judgement.js";
describe('Judgement', () => {
        ...
        describe('constructor()', () => {
            describe('sync', () => {
                test('constructor()', () => {
                    ...
                    expect(target).toThrow(`fnは関数またはPromiseであるべきです。`);
                });
            ...
            describe('async', () => {
                describe('awaitせずにisPassなどを参照しようとする(constructor(async(pass,fail)=>pass(1)))', () => {
                    test.each([
                        ...
                        const judgement = new Judgement(async(pass,fail)=>0);// asyncなのにawaitしない
                        expect(judgement).toBeInstanceOf(Judgement);
                        expect(judgement.result).toBeInstanceOf(Promise);
                        try {isMethod ? judgement[name]() : judgement[name]; expect.unreachable();} catch (e) {
                    ...
                    /*
                    test('isPass', async() => {
                        const judgement = new Judgement(async(pass,fail)=>0);
                        expect(judgement).toBeInstanceOf(Judgement);
                        expect(judgement.result).toBeInstanceOf(Promise);
                        try {judgement.isPass; expect.unreachable();} catch (e) {
                            expect(e).toBeInstanceOf(JudgementAsyncError);
