...
//    pass(cb) {throwFn(cb); if(this.isPass){this._.on.pass = cb}; return this;}
//    value(v) {if(this.isFail){this._.on.value=v}; return this;}
//    cause(cb) {throwFn(cb); if(this.isFail){this._.on.cause = cb}; return this;}
...
//    throw(fn) {if(isFn(fn)){fn(this.#error())}; this.#throw();}
    throw(fn) {
        const e = new JudgementFailError(`失敗。`, this.cause); // 一度だけ生成
        if(isFn(fn)){ fn(e) }; 
        throw e; // 同じインスタンスを投げる
    }
